...
export default function Dashboard(props) {
  ...
  const getActiveRoute = (routes, currentPath) => {
    ...
    for (let i = 0; i < routes.length; i++) {
      if (routes[i].collapse) {
        ...
        if (
          currentPath.startsWith(routes[i].path)
        ) {
          return routes[i].name;
        }
      }
    }
    return activeRoute || 'Dashboard'; // fallback to Dashboard
  };
  ...
  const getRoutes = (routes) => {
    return routes.map((route, key) => {
      if (route.path && !route.public) {
        let path = route.path;
        // Ajustar paths para layouts anidados
        if (window.location.pathname.startsWith('/dashboard')) {
          if (route.path === '/dashboard') path = '';
          else if (route.path === '/profile') path = 'profile';
          else if (route.path.startsWith('/admin')) return null; // No mostrar admin-only aquí
        } else if (window.location.pathname.startsWith('/profile')) {
          if (route.path === '/profile') path = '';
          else return null;
